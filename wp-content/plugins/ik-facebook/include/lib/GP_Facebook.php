...
class GP_Facebook{	
	...
	function generateAccessToken(){
		if(!isset($this->access_token)){
			...
			$response = $this->root->fetchUrl("https://graph.facebook.com/oauth/access_token?type=client_cred&client_id={$app_id}&client_secret={$app_secret}", true);
		
			// check for an error object first
			// if there is one, return it
			// the output from here will be used in the diagnostics test page
			// if there isn't one, fall through to the next steps
			if( !empty($response->error) ){
				return $response;
			}
			
			$the_access_token = !empty( $response->access_token )
								  ? $response->access_token
								  : $response;
								  
			$this->access_token = "access_token=$the_access_token";
		}
	...
	*/
	...
	function formatAsGraphObject($feed = array()){		
		...
		foreach($feed as $feed_item){
			...
			$formatted_feed[$i]['name'] = $feed_item->post_title;
			$formatted_feed[$i]['caption'] = !empty($post_meta['_ikcf_caption']) ? $post_meta['_ikcf_caption'][0] : '';
			$formatted_feed[$i]['link'] = !empty($post_meta['_ikcf_link']) ? $post_meta['_ikcf_link'][0] : '';
			$formatted_feed[$i]['caption'] = !empty($post_meta['_ikcf_caption']) ? $post_meta['_ikcf_caption'][0] : '';
			$formatted_feed[$i]['description'] = !empty($post_meta['_ikcf_description']) ? $post_meta['_ikcf_description'][0] : '';
			$formatted_feed[$i]['type'] = !empty($post_meta['_ikcf_content_type']) ? $post_meta['_ikcf_content_type'][0] : ''; //ie Event
			$formatted_feed[$i]['item_type'] = !empty($post_meta['_ikcf_item_type']) ? $post_meta['_ikcf_item_type'][0] : ''; //ie Video or nothing
			$formatted_feed[$i]['item_source'] = !empty($post_meta['_ikcf_content_source']) ? $post_meta['_ikcf_content_source'][0] : '';//ie Path to Video or nothing
			$formatted_feed[$i]['status_type'] = !empty($post_meta['_ikcf_status_type']) ? $post_meta['_ikcf_status_type'][0] : '';
			$formatted_feed[$i]['created_time'] = !empty($post_meta['_ikcf_created_time']) ? $post_meta['_ikcf_created_time'][0] : '';
			$formatted_feed[$i]['updated_time'] = !empty($post_meta['_ikcf_updated_time']) ? $post_meta['_ikcf_updated_time'][0] : '';
			$formatted_feed[$i]['comments'] = $feed_item->comments_array;			
			$formatted_feed[$i]['num_likes'] = !empty($post_meta['_ikcf_like_count']) ? $post_meta['_ikcf_like_count'][0] : '';	
			$formatted_feed[$i]['num_comments'] = !empty($post_meta['_ikcf_comment_count']) ? $post_meta['_ikcf_comment_count'][0] : '';
			
			//pass back content type
			$formatted_feed[$i]['content_type'] = !empty($post_meta['_ikcf_content_type']) ? $post_meta['_ikcf_content_type'][0] : '';
			
			//event data
			$formatted_feed[$i]['start_time'] = !empty($post_meta['_ikcf_start_time']) ? $post_meta['_ikcf_start_time'][0] : '';
			$formatted_feed[$i]['end_time'] = !empty($post_meta['_ikcf_end_time']) ? $post_meta['_ikcf_end_time'][0] : '';
			$formatted_feed[$i]['location'] = !empty($post_meta['_ikcf_location']) ? $post_meta['_ikcf_location'][0] : '';
			$formatted_feed[$i]['timezone'] = !empty($post_meta['_ikcf_timezone']) ? $post_meta['_ikcf_timezone'][0] : '';
	...
	function add_pro_user_avatars($content = "", $item = array()){			
	...
	function ik_fb_output_feed_shortcode($atts){
		...
		// If "Classic Theme" active, force Like button to use dark color scheme
		$active_theme = get_option('ik_fb_feed_theme', '');
	...
	function ik_fb_output_gallery_shortcode($atts){			
		...
		return $this->ik_fb_output_gallery($id, $size, $show_name, $title, $num_photos, $masonry);				
	}
	...
	public function ik_fb_output_gallery($id = '', $size = '320x180', $show_name = true, $the_title = null, $num_photos = false, $masonry = true)
	{
		...
		if ( $masonry ) {
			echo '<div class="ik_fb_gallery_standard wpsp_masonry_gallery">';
		} else {
	...
	public function build_event_feed_line_item_html($item){		
		...
		if($event_id) {			
			...
			if($this->is_pro){		
				$line_item = $this->add_pro_user_avatars($line_item, $item) . " ";
			}
	...
	function ikfb_build_message($item, $avatars = "" , $message_html )
	{
		$shortened = false;		
	
		//add avatar for pro users
		if($this->is_pro){
			// the new content will be added to the current $avatars content,
			// so it won't really be lost here
			$avatars = $this->add_pro_user_avatars($avatars, $item) . " ";
		}		
		...
		// NOTE: there was once a make_clickable($message) here. it caused problems
		// so its been replaced with this filter
		$message = apply_filters('wp_social_fb_post_message', $message, $item, $limit);		
		...
		$output = str_replace('{ikfb:feed_item:message}', $message, $message_html);			
