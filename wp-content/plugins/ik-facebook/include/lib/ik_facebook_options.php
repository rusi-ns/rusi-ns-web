...
class ikFacebookOptions
{
	...
	function output_style_options(){
		...
		if($this->is_pro){
			$ikfb_themes['classic_light'] = 'Classic Theme - Light';
			$ikfb_themes['classic_dark'] = 'Classic Theme - Dark';
	...
	function shortcode_generator_page()
	{
		...
		?>				
		<div id="wp-social-shortcode-generator wp-content-wrap">
	...
	function run_diagnostics($page_ids)
	{
		...
		//if the access_token didn't work, it will be an error object with a reason why
		if ( !empty($access_token->error)){
			$results['keys_work'] = false;
			$results['access_token'] = $access_token->error;
		} else {
...
?>
	<table class="table" id="plugin_status_table" cellpadding="0" cellspacing="0">
		<tbody>
			...
			<tr class="fail">
				<td><img src="<?php echo $this->root->plugins_url . 'include/img/x-button.png'; ?>" alt="FAIL" /></td>
				<td>Unable to Connect To Facebook Graph API, with the following error: <?php echo $diagnostics_results['access_token']->message; ?></td>
			</tr>
			<?php endif; ?>
